...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def send_multiple_images(
        ...
        # Mirror send()'s degraded-path short-circuit (line ~4622): without
        # this, a mid-turn polling reconnect leaves send_media_group/
        # send_image_file attempting real network I/O against a degraded
        # connection, each one hanging toward _MEDIA_SEND_READ_TIMEOUT
        # instead of failing fast — the "only one image sporadically sends"
        # symptom. Fail fast and let the per-image loop log each skip.
        if getattr(self, "_send_path_degraded", False):
            logger.warning(
                "[%s] Send path degraded, skipping media group for %d image(s)",
    ...
    async def send_image_file(
        ...
        **kwargs,
        ...
        # Same degraded-path short-circuit as send() (line ~4622); this
        # method has no equivalent check, so per-image sends in a batch
        # after a mid-turn reconnect were each attempting real I/O and
        # timing out individually instead of failing fast.
        if getattr(self, "_send_path_degraded", False):
            return SendResult(success=False, error="send_path_degraded", retryable=True)
